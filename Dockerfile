...
RUN zypper --non-interactive install -y -l git make ccache gcc-c++ tcl-devel tk-devel json_spirit-devel librsvg2-devel gsl-devel cairo-devel pango-devel readline-devel gdbm-devel libXss-devel ccache xvfb-run xorg-x11-fonts xauth which soci-devel unittest++-devel libxml2-devel libtiff-devel octave-cli 
